...
@Composable
fun QRPaymentConfirmationRoot(
    onNavigateBack: () -> Unit,
    viewModel: QRPaymentConfirmationViewModel = koinViewModel(),
) {
    val state by viewModel.state.collectAsStateWithLifecycle()

    ObserveAsEvents(viewModel.events) { event ->
        when (event) {
            is QRPaymentConfirmationEvent.NavigateBack -> onNavigateBack()
        }
    }
...
fun QRPaymentConfirmationScreen(
    ...
    onAction: (QRPaymentConfirmationAction) -> Unit,
) {
    CampusWalletTheme {
        Scaffold(
            topBar = {
                TopAppBar(
                    ...
                    navigationIcon = {
                        IconButton(onClick = { onAction(QRPaymentConfirmationAction.OnBackClick) }) {
                            Icon(
            ...
            Box(
                ...
                if (state.isLoading) {
                    ...
                    Column(
                        ...
                        Text(
                            text = "Confirm Payment Screen Content",
